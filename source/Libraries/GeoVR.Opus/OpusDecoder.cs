...
{   // https://github.com/DevJohnC/Opus.NET updated to libopus 1.3.1
    ...
    public class OpusDecoder : IDisposable
    {
        ...
        public static OpusDecoder Create(int outputSampleRate, int outputChannels)
        {
            ...
            IntPtr error;
            IntPtr decoder = Native.opus_decoder_create(outputSampleRate, outputChannels, out error);
        ...
        public unsafe byte[] Decode(byte[] inputOpusData, int dataLength, out int decodedLength)
        {
            ...
            fixed (byte* bdec = decoded)
            {
                ...
                if (inputOpusData != null)
                    length = Native.opus_decode(_decoder, inputOpusData, dataLength, decodedPtr, frameCount, 0);
                else
                    length = Native.opus_decode(_decoder, null, 0, decodedPtr, frameCount, (ForwardErrorCorrection) ? 1 : 0);
            }
        ...
        public void Dispose()
        {
            ...
            {
                Native.opus_decoder_destroy(_decoder);
