...
namespace GeoVR.Opus
{   // https://github.com/DevJohnC/Opus.NET updated to libopus 1.3.1
    /// <summary>
    /// Opus audio encoder.
    /// </summary>
    public class OpusEncoder : IDisposable
    {
        ...
        public static OpusEncoder Create(int inputSamplingRate, int inputChannels, Application application)
        {
            ...
            IntPtr error;
            IntPtr encoder = Native.opus_encoder_create(inputSamplingRate, inputChannels, (int)application, out error);
        ...
        public unsafe byte[] Encode(byte[] inputPcmSamples, int sampleLength, out int encodedLength)
        {
            ...
            fixed (byte* benc = encoded)
            {
                ...
                length = Native.opus_encode(_encoder, inputPcmSamples, frames, encodedPtr, sampleLength);
            }
        ...
        public int Bitrate
        {
            get
            {
                if (disposed)
                    throw new ObjectDisposedException("OpusEncoder");
                int bitrate;
                var ret = Native.opus_encoder_ctl(_encoder, Ctl.GetBitrateRequest, out bitrate);
                if (ret < 0)
            ...
            set
            {
                if (disposed)
                    throw new ObjectDisposedException("OpusEncoder");
                var ret = Native.opus_encoder_ctl(_encoder, Ctl.SetBitrateRequest, value);
                if (ret < 0)
                    throw new Exception("Encoder error - " + ((Errors)ret).ToString());
            }
        }
        ...
        public bool ForwardErrorCorrection
        {
            get
            {
                ...
                int fec;
                int ret = Native.opus_encoder_ctl(_encoder, Ctl.GetInbandFECRequest, out fec);
                if (ret < 0)
                    throw new Exception("Encoder error - " + ((Errors)ret).ToString());
            ...
            set
            {
                ...
                var ret = Native.opus_encoder_ctl(_encoder, Ctl.SetInbandFECRequest, value ? 1 : 0);
                if (ret < 0)
                    throw new Exception("Encoder error - " + ((Errors)ret).ToString());
            }
        }
